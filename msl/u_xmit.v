//u_xmit
`include "inc.h"
module u_xmit #(parameter width = `width)(
    input                   uart_clk,
    input                   sys_rst,
    input                   xmit_h,
    ...
    always @(posedge uart_clk or negedge sys_rst) begin
        if (!sys_rst) begin
            ...
            if (ct == idle)
                ...
                index <= index + 1;
            if (nt==idle && ct==stop)
                xmit_done_h <= 1'b1;        // set at end of stop
            else if (ct == idle && xmit_h)
                xmit_done_h <= 1'b0;        // clear when new TX starts
            xmit_active <= (ct != idle);
        end
    end
    ...
    always @(*) begin
        ...
        case (ct)
            idle:  begin
                out = 1;
                nt  = xmit_h ? start : idle;
            end
            start: begin
                out = 0;
                nt  = (count == 15) ? data : start;
            end
            data:  begin
